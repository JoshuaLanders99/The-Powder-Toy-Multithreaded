...
namespace
{
	...
	public:
		...
		~SimulationThreadPool();
		void RunParallel(SimulationImpl *sim, int start, int end, int active);
		int GetThreadCount() const { return workers.size(); }
	};

	static SimulationThreadPool globalThreadPool;
	...
	struct SimulationImpl : public Simulation
	{
		...
		int GetThreadCount() const final override;
		int GetActiveSimulationThreads() const final override;
	};
	...
	SimulationThreadPool::SimulationThreadPool()
	{
		...
		for (int i = 0; i < numThreads; ++i)
		{
			...
			w->thread = std::thread([this, workerPtr = w.get()]() {
				while (true)
				{
					std::unique_lock<std::mutex> lock(workerPtr->mutex);
					workerPtr->cv.wait(lock, [workerPtr]() { return workerPtr->hasWork || workerPtr->stop; });
					if (workerPtr->stop)
					{
						break;
					}
					...
					// Do work
					workerPtr->sim->UpdateParticlesSerial(workerPtr->start, workerPtr->end);

					workerPtr->hasWork = false;
					lock.unlock();
	...
	void SimulationThreadPool::RunParallel(SimulationImpl *sim, int start, int end, int active)
	{
		int totalRange = std::min(end, active) - start;
		if (totalRange <= 0)
		{
			return;
		}
		...
		int chunkSize = (totalRange + numWorkers - 1) / numWorkers;
		if (chunkSize < 1000)
		{
			// Too small, run sequentially on main thread
			sim->UpdateParticlesSerial(start, end);
			return;
		}
		...
		for (int i = 0; i < numWorkers; ++i)
		{
			...
			int wEnd = std::min(wStart + chunkSize, end);
		...
		joinCv.wait(lock, [this]() { return activeWorkers == 0; });
	}

	int SimulationImpl::GetThreadCount() const
	{
		return globalThreadPool.GetThreadCount();
	}

	int SimulationImpl::GetActiveSimulationThreads() const
	{
		if (ensureDeterminism)
			return 1;
		int totalRange = parts.active;
		int numWorkers = globalThreadPool.GetThreadCount();
		int chunkSize = (totalRange + numWorkers - 1) / numWorkers;
		if (chunkSize < 1000)
			return 1;
		return numWorkers;
...
void SimulationImpl::UpdateParticles(int start, int end)
{
	...
	{
		globalThreadPool.RunParallel(this, start, end, parts.active);
	}
}
